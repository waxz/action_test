...
nohup bash -c "while true; do cloudflared tunnel --url localhost:80   > /tmp/cloudflared-web.out 2>&1 ;flock -x  /tmp/cloudflared-web.out  truncate -s 0 /tmp/cloudflared-web.out;  done " > /tmp/cloudflared.nohup-web.out 2>&1 &
...
if [ -s /tmp/cloudflared-web.out ]; then
    cloudflared_web_url_new=$(flock -s  /tmp/cloudflared-web.out   cat /tmp/cloudflared-web.out | grep -oE "https://[a-zA-Z0-9.-]+\.trycloudflare\.com");
    if [ -z "$cloudflared_web_url_new" ]; then
        echo "No cloudflared URL found";
    else
        cloudflared_web_url=$cloudflared_web_url_new
    fi
fi
...
JSON_STRING=$( jq -n \
            ...
            --arg cloudflared_web_url "$cloudflared_web_url" \
            --arg pinggy_url "$pinggy_url" \
            --arg up_time "$UP_TIME" \
            --arg sshx_url "$sshx_url"\
            --arg pinggy_ssh_url "$pinggy_ssh_url" \
            '{up_time: $up_time, sshx_url: $sshx_url, cloudflared_url: $cloudflared_url, cloudflared_ssh_url: $cloudflared_ssh_url, cloudflared_web_url: $cloudflared_web_url, pinggy_url: $pinggy_url, pinggy_ssh_url: $pinggy_ssh_url}' )
